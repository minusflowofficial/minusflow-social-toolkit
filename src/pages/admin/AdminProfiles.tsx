  ...
  if (selectedUser) {
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <Button variant="ghost" size="sm" className="mb-4 gap-1.5" onClick={() => setSelectedUser(null)}>
          <ArrowLeft className="h-4 w-4" /> Back to Users
        </Button>

        {/* User Profile Card */}
        <div className="mb-6 overflow-hidden rounded-2xl border border-border/50 bg-card">
          <div className="h-28 bg-gradient-to-r from-primary/30 via-primary/10 to-accent/20" />
          <div className="relative px-6 pb-6">
            <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-end">
              ...
              <div className="flex-1 pt-2">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${
                    selectedUser.is_suspended ? "bg-destructive/10 text-destructive" : "bg-green-500/10 text-green-500"
                  }`}>
              ...
              <Button
                ...
                className="gap-1.5"
                onClick={() => {
                  setNotifyUserId(selectedUser.id);
        ...
        <div className="mb-6 grid gap-3 grid-cols-2 sm:grid-cols-4">
          {[
            ...
            { label: "Pages Visited", value: userPageViews?.uniquePaths ?? 0, icon: Activity, color: "text-green-500 bg-green-500/10" },
            {
              ...
              value: `${bounceRate}%`,
              icon: Activity,
              color: bounceRate > 20 ? "text-yellow-500 bg-yellow-500/10" : "text-emerald-500 bg-emerald-500/10",
            },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
        ...
        {showNotifyModal && (
          ...
          >
            <motion.div
              ...
              className="mx-4 w-full max-w-md rounded-2xl border border-border/50 bg-card p-6 shadow-2xl"
            >
              ...
              <div className="space-y-3">
                ...
                <Button
                  ...
                  disabled={sendNotification.isPending || !notifyTitle || !notifyMessage}
