...
import { Download, Loader2, AlertCircle, Film, Music, Copy, Check, DownloadCloud, Globe } from "lucide-react";
...
interface BulkItem { url: string; status: "pending" | "loading" | "success" | "error"; result?: DouyinResult; error?: string; }
      ...
      <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {item.result.download_links.map((link, j) => (
                      <div key={j} className="flex items-center gap-1">
                        <Button size="sm" onClick={() => { triggerDownload(buildProxyUrl(link.url, item.result!.title || "video", link.quality)); toast.success("Download started!"); }} className={`gap-1.5 text-white ${link.format === "mp3" ? "bg-orange-500 hover:bg-orange-600" : "bg-[#fe2c55] hover:bg-[#e0264c]"}`}>
        ...
        {!processing && successItems.length > 0 && (
          <Button
            ...
            className="w-full h-12 rounded-xl font-semibold bg-[#fe2c55] hover:bg-[#e0264c] text-white gap-2"
          >
